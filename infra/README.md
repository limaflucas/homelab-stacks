...
*   **PostgreSQL HA Cluster** (`pgsql-primary`, `pgsql-standby`, `pgpool`): A highly available PostgreSQL 17 cluster with native streaming replication (1 primary node, 2 standbys) and Pgpool-II for query load-balancing/routing.
    *   *Note:* PostgreSQL database nodes are configured for `max_connections=200`, `shared_buffers=512MB`, and `work_mem=8MB`. Pgpool-II (`PGPOOL_MAX_POOL=2`, `PGPOOL_NUM_INIT_CHILDREN=70`) is used to route and load-balance client connections directly to the backend databases.
